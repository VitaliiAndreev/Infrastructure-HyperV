...
| `Stop-VmProcessesUsingPath` | Sends SIGTERM under sudo to every VM process whose open files, cwd, executable, or memory mappings touch `<Path>`, then polls `kill -0` at 0.5s intervals for up to `<GraceSeconds>` and returns `{ TerminatedPids, KilledPids, StillAlive }`. The scanner prefers `lsof +D`, falls back to `fuser -m`, then walks `/proc/*/{exe,cwd,maps}`. SIGTERM-only in this release - `KilledPids` is always empty and any survivor in `StillAlive` causes the cmdlet to throw; the SIGKILL escalation lands in a follow-up release. Path is validated host-side (absolute, no `..`, no NUL, no single quote) before any SSH call. |
